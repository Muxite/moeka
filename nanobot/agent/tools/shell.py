...
class ExecTool(Tool):
    ...
    # Always applied, never user-tunable: writes to these files corrupt
    # internal state (history.jsonl / .dream_cursor) and crash /dream (#2989).
    _INTERNAL_DENY_PATTERNS: list[str] = [
    ...
    async def execute(
        ...
        if self._SUDO_PATTERN.search(command):
            if not self.allow_sudo:
                logger.warning("exec: sudo command rejected (allow_sudo=False): {!r}", command[:120])
                return (
