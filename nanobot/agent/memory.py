...
class MemoryStore:
    ...
    def append_history(
        ...
        *,
        ...
        with self._append_lock:
            ...
            # moeka: mirror the entry into the semantic VecStore (if wired) so
            # hybrid history retrieval can surface it later.
            if self.vec_store:
    ...
    def reindex_memory(self) -> None:
        """moeka: re-index MEMORY.md into the semantic VecStore after edits.

        Dream runs edit MEMORY.md via the file tools; this refreshes the
        vector index so hybrid memory retrieval reflects the latest content.
        No-op when no VecStore is wired or it is unavailable.
        """
        if self.vec_store and self.vec_store.available:
    ...
    def prune_dream_sessions(sessions: SessionManager, *, keep: int = 10) -> None:
        """Remove the oldest Dream sessions from the store, keeping only the
        ...
        try:
            ...
            logger.warning("Failed to enumerate dream sessions for pruning", exc_info=True)
            return
        stale_keys = [row[0] for row in rows[keep:]]
        for key in stale_keys:
            try:
                sessions.delete_session(key)
                logger.debug("Pruned old dream session: {}", key)
            except Exception:
                logger.warning("Failed to prune dream session {}", key)
