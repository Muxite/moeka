...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / "tool-results" / "test_runner" / "call_big.txt").exists()
...
async def test_read_file_result_is_not_offloaded(tmp_path):
    ...
    offload_dir = tmp_path / "tool-results"
...
async def test_length_recovery_continues_from_truncated_output():
    ...
    assert result.stop_reason == "completed"
    # Length-recovery segments are stitched together into one continuous
    # final answer rather than only keeping the last segment.
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from agent.runner_helpers import make_run_spec
    from nanobot.agent.runner import _MAX_LENGTH_RECOVERIES, AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from nanobot.agent.context_governance import BACKFILL_CONTENT
    from nanobot.agent.loop import AgentLoop
    from nanobot.bus.events import InboundMessage
    from nanobot.bus.queue import MessageBus
