...
class TestProactiveAutoCompact:
    ...
    async def test_idle_tick_survives_session_removed_during_listing(
        ...
        # Simulate a session being deleted concurrently, between the idle-scan
        # listing and the later per-session get_or_create()/archive step.
        original_list_sessions = loop.sessions.list_sessions

        def list_sessions_then_remove():
            rows = original_list_sessions()
            loop.sessions.delete_session(removed.key)
            return rows

        monkeypatch.setattr(loop.sessions, "list_sessions", list_sessions_then_remove)
